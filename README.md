...
**调研深度**：`--depth` 一条指令同时控制检索轮数与并行子代理数。

| 档位 | 检索轮数 | 并行子代理 | 适用场景 |
|---|---|---|---|
| `quick` | 2 | 2 | 先摸清方向，几分钟出结果 |
| `standard`（默认） | 3 | 3 | 常规调研 |
| `deep` | 5 | 3 | 尽可能穷尽，耗时与检索额度消耗最高 |

> 轮数不宜设高：报告质量通常在前几轮就收敛，之后的迭代多为重复检索。一轮里主管可以并行派发多个子代理，因此「检索轮数」不等于检索次数。
